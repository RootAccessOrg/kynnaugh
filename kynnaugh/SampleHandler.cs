			...
			foreach (var kvp in sampleStore)
			{
				lock (kvp.Value)
				{
					...
					if (threshold > kvp.Value.LastUpdatedTime)
					{
						...
						TranscribeSamples(samples, id.Channels);
					}
				}
			}
		}

		public void TranscribeSamples(short[] samples, int channels)
		{
			...
			byte[] speechData = PcmToFlac.Convert(sampleData, channels);
			Console.WriteLine(speechData.Length + " bytes of FLAC received");

			SpeechTranscriber transcriber = new SpeechTranscriber();
			string text = transcriber.TranscribeSpeech(speechData);

			if (text != null)
			{
				TS3Functions.printMessageToCurrentTab(Encoding.UTF8.GetString(Encoding.Default.GetBytes(text)));
			}
		}
	}
